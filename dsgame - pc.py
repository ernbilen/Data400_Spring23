...
# Author: Eren Bilen v01, Aug 10, 2022
...
help_text = """
You have two resources that you need to keep track of: Data Analysis (DA) points, and user trust. You begin with 25 DA points and 25 user trust points.
...
def belowed_but_outdated():
	if (da<15) & (trust>=25) & (da>0) & (trust>0):
	  return True
def bleeding_edge_bleeding_users():
	if (da>=25) & (trust <15) & (da>0) & (trust>0):
	  return True
def middle_of_the_road():
	if ((da<25) & (da>=15)) & (da>0) & (trust>0) | ((trust<25) & (trust >=15) & (da>0) & (trust>0)):
	  return True
def total_victory():
	if (da>=25) & (trust>=25):
	  return True
...
## prints the welcome text with pauses in between
